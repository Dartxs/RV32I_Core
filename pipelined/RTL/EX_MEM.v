module EX_MEM(
    input clk, rst_n,
    input mem_writeE, mem_readE, reg_writeE,
    input [1:0] writeback_ctrlE,
    input [2:0] funct3E,
    input [4:0] rdE,
    input [31:0] ALU_outE, PCp4E, op2E, immE,
    output reg mem_writeM, mem_readM, reg_writeM,
    output reg [1:0] writeback_ctrlM,
    output reg [2:0] funct3M,
    output reg [4:0] rdM,
    output reg [31:0] ALU_outM, PCp4M, op2M, immM
    );

    always @(posedge clk) begin
        if(!rst_n) begin
            {mem_writeM, mem_readM, reg_writeM} <= 3'b0;
            writeback_ctrlM <= 2'b0;
            funct3M <= 3'b0;
            rdM <= 5'b0;
            {ALU_outM, PCp4M, op2M, immM} <= 128'b0;
        end else begin
            {mem_writeM, mem_readM, reg_writeM} <= {mem_writeE, mem_readE, reg_writeE};
            writeback_ctrlM <= writeback_ctrlE;
            funct3M <= funct3E;
            rdM <= rdE;
            {ALU_outM, PCp4M, op2M, immM} <= {ALU_outE, PCp4E, op2E, immE};
        end
    end

endmodule
